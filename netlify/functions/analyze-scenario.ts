...
function buildFallbackReport(scenario: string, transactions: TxInput[]) {
  ...
  return {
    ...
    keyFindings: [
      ...
      riskTxs.length > 0 ? `最大风险交易：¥${Math.max(...riskTxs.map((t) => t.amount)).toLocaleString()}` : '未发现明显风险交易',
    ],
    riskAlerts: riskTxs.slice(0, 5).map((t) => `${t.txDate.slice(0, 10)} ${t.counterparty} ¥${t.amount.toLocaleString()} ${t.riskTags.join(',')}`),
    timeline: transactions.slice(0, 10).map((t) => ({
      date: t.txDate.slice(0, 10),
      event: `${t.counterparty} - ${t.summary}`,
      amount: t.amount,
    })),
    recommendations: ['建议重点关注已标记的风险交易', '可进一步追踪频繁往来对象', '建议补充缺失时段的流水以完善分析'],
...
export const handler: Handler = async (event) => {
  ...
  try {
    const { transactions, scenario = 'general', subjects = [] } = JSON.parse(event.body || '{}')
    if (!transactions?.length) {
      return { statusCode: 400, body: JSON.stringify({ error: '缺少交易数据' }) }
    }

    const summaryTxs: TxInput[] = transactions.slice(0, 200).map((t: TxInput) => ({
      id: t.id,
      txDate: t.txDate,
    ...
    const riskStats = transactions.filter((t: TxInput) => t.isRisk)
    ...
    if (!API_KEY) {
      return {
        statusCode: 200,
        body: JSON.stringify(buildFallbackReport(scenario, transactions)),
      }
    }
...
追踪对象：${subjects.map((s: { name: string }) => s.name).join('、') || '无'}
...
${JSON.stringify(summaryTxs, null, 0)}`
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: err instanceof Error ? err.message : '分析失败' }),
    }
  }
}
